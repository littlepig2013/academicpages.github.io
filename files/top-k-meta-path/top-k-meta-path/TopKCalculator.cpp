...
set<int> TopKCalculator::getSimilarNodes(int eid, map<int, HIN_Node> & hin_nodes_, bool output_flag)
{
...
void TopKCalculator::saveToFile(vector<pair<double, vector<int>>> topKMetaPaths, string file_name){
	...
	for(int i = 0; i < topKMetaPaths.size(); i++){
		double tfidf = topKMetaPaths[i].first;
		vector<int> currMetaPath = topKMetaPaths[i].second;
		topKMetaPathsOut << tfidf;
		for(int j = 0; j < currMetaPath.size(); j++){
			topKMetaPathsOut << "\t" << currMetaPath[j];
		}
		topKMetaPathsOut << endl;
...
vector<vector<int>> TopKCalculator::loadMetaPaths(string file_name){
	...
	while(getline(topKMetaPathsIn, line)){
		...
		for(unsigned int j = 1; j < strs.size(); j++){
			meta_paths.back().push_back(atoi(strs[j].c_str()));
		}
